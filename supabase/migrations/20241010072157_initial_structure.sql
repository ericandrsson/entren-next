...
BEGIN
  RETURN EXISTS (
    ...
    WHERE user_roles.user_id = user_has_role.user_id
      AND user_roles.role = user_has_role.required_role
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION public.assign_role(user_id UUID, role TEXT)
RETURNS VOID AS $$
BEGIN
  INSERT INTO public.user_roles (user_id, role)
  VALUES (assign_role.user_id, assign_role.role)
  ON CONFLICT (user_id, role) DO NOTHING;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION public.remove_role(user_id UUID, role TEXT)
RETURNS VOID AS $$
BEGIN
  DELETE FROM public.user_roles
  WHERE user_roles.user_id = remove_role.user_id
    AND user_roles.role = remove_role.role;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
